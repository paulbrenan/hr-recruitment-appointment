...
<div class="wrap">
  ...
  <div class="body">
    ...
    <p>
      We are pleased to inform you that you have <strong>passed</strong> the initial screening for the
      <strong>{{ $posting->title }}</strong> position.
    </p>

    <div class="txn">
      <div class="lbl">Your Ranking</div>
      <div class="num">#{{ $rank }} of {{ $total }} applicants</div>
    </div>

    <div class="section-title">Ranking Summary</div>
    <div class="detail-row"><span class="lbl">Position</span><span class="val">{{ $posting->title }}</span></div>
    <div class="detail-row"><span class="lbl">Rank</span><span class="val">#{{ $rank }} of {{ $total }}</span></div>
    <div class="detail-row"><span class="lbl">Weighted Score</span><span class="val">{{ $score }} / 100</span></div>

    <div class="section-title">Next Step: {{ $typeLabel }}</div>
    <div class="detail-row"><span class="lbl">Date &amp; Time</span><span class="val">{{ $when }}</span></div>
    ...
    <div class="btn-wrap">
      <a href="{{ url('/job-postings/' . $posting->id) }}" class="btn">View Job Posting</a>
    </div>
