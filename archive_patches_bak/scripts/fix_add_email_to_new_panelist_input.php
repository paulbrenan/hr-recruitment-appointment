<?php
/**
 * fix_add_email_to_new_panelist_input.php
 *
 * The "Add panelist" inline input on the job posting create/edit form
 * only ever captured a NAME -- Panelist::create(['name' => $name]) had
 * no email at all. Any panelist added this way had nothing to send a
 * Mailpit test invite to.
 *
 * Adds:
 *   1. An email input next to the name input in the dynamically-added
 *      row (form.blade.php + its addPanelistBtn JS).
 *   2. syncPanelists() now reads new_panelist_emails[] alongside
 *      new_panelist_names[] and saves both.
 *   3. Cleans up a stale hint line left over from the earlier
 *      fix_remove_panelist_available_checkbox.php patch that still said
 *      "Tick Available if they are available" even though that checkbox
 *      no longer exists.
 *
 * HOW TO RUN:
 *   php fix_add_email_to_new_panelist_input.php   (from project root)
...
apply_patch(
    ...
    `        div.innerHTML = \`
        ...
        \`;`,
    'form.blade.php: email input added to new-panelist row'
);
