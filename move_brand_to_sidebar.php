<?php
/**
 * move_brand_to_sidebar.php
 *
 * One-shot script: full-file replace of resources/views/layouts/app.blade.php.
 * Verifies the file matches the exact expected current content (the result of
 * update_admin_header_sidebar.php + separate_header_sidebar.php) before writing.
 *
 * Structural change:
 * - The header no longer spans the full page width above everything. The
 *   sidebar is now a full-height first column (brand "HR Recruitment" lives
 *   at its top instead of in the header), and the header becomes a slim bar
 *   confined to the content area beside it -- matching the reference image's
 *   layout (full-height dark sidebar with its own brand, separate header bar
 *   only over the content).
 * - Header (left to right): hamburger toggle, home button, then a live
 *   date/time clock (e.g. "Tuesday, June 30, 2026 at 10:28:36 AM"),
 *   updating every second via vanilla JS.
 * - Header (right side): unchanged, just the fullscreen toggle.
 * - Sidebar collapse/expand logic unchanged -- same classes, same
 *   localStorage persistence, same tooltip re-init -- just the brand row
 *   now also hides its label text when collapsed, the same way nav links do.
 * - No color palette changes.
 *
 * Usage: place this file in the project root (same folder as artisan) and run:
 *   php move_brand_to_sidebar.php
...
echo "\nDone.\n";
echo "Next steps:\n";
echo "  1. Refresh any admin page -- confirm the sidebar is now full-height with\n";
echo "     'HR Recruitment' at its top, and the header is a slim bar beside it\n";
echo "     containing the hamburger, home button, and a live-updating date/time.\n";
echo "  2. Confirm collapsing the sidebar hides the brand label text too (icon stays).\n";
echo "  3. Confirm the date/time updates every second without a page refresh.\n";
echo "  4. Delete this script once you've confirmed everything works.\n";
